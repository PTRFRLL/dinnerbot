...
const getLatestWin = async(playerId) => {
    try{
        ...
        const winning = results.filter(match => {
            return match.place == 1 && (match.matchType === 'official' || match.matchType === 'seasonal');
        });
        if(winning.length === 0){
            logger.debug(`No win found in last 50 matches`)
            return undefined;
        }else{
            logger.debug(`Latest win`);
            const win = winning.shift();
            logger.debug(JSON.stringify(win));
            return win;
        }
    }catch(e){
        logger.error(e);
