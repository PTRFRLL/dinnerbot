...
const isAuth = (message) => {
  try{
    let AUTH_USERS = process.env.AUTH_USERS;
    let AUTH_ROLES = process.env.AUTH_ROLES;

    AUTH_ROLES = AUTH_ROLES.split(',').map(role => role.trim());
    AUTH_USERS = AUTH_USERS.split(',').map(user => user.trim());
    logger.debug(`AUTH_USERS: ${AUTH_USERS}`);
    logger.debug(`AUTH_ROLES: ${AUTH_ROLES}`);
    if (
      AUTH_USERS.users.includes(message.author.id) ||
      message.member.roles.cache.some(r => AUTH_ROLES.includes(r.name))
    ) {
      return true;
    } else {
      return false
