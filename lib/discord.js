...
const {logNonWin, logUserWins, getWins, getLatestNonWin, removeMessageFromDB} = require('./db_utils.js')
  	...
  	if (message.channel.id === CONFIG.app.DISCORD_CHANNEL) {
	    ...
	    if(attachment){
			...
			if(CONFIG.app.AUTH_USERS.users.includes(message.author.id) || message.member.roles.some(r => CONFIG.app.AUTH_USERS.roles.includes(r.name))){
				...
				let res = await logUserWins(users);
...
let winners = [];
let spectators = [];
const presenceUpdate = (old, updated) => {
