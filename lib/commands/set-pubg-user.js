...
module.exports = {
	...
	async execute(message, args) {
        ...
        if(currentUser && currentUser.pubg_username === username){
            ...
            message.reply(`You're PUBG username is already set as **${currentUser.pubg_username}**. \nDo you want to replace it with **${username}**?`).then(async msg => {
                const responses = ['👍', '👎'];
                const filter = (reaction, user) => {
                    return  user.id === message.author.id && responses.some(emoji => emoji === reaction.emoji.name)
                };
                Promise.all(responses.map(emoji => msg.react(emoji)))
