...
const isRecentWin = (date, min = 45) => {
    ...
    if(isWithinInterval(statDate, {
      ...
      logger.debug(`Stat date NOT within last ${min} min: ${statDate.toISOString()}`);
      return false;
    }
}
...
const findRecentStats = async(pubId, statusMessage) => {
  ...
  if(stats && recent){
    ...
    logger.info(`Stats found but not recent enough, stat date is ${new Date(stats.date).toISOString()}, trying again...`);
