...
const updatePlayerUsername = async(user, PUBGUsername, pubg_id) => {
  ...
  if(dbUser){
    logger.log(`${user.username} found, updating PUBG username to ${PUBGUsername} (${pubg_id})`);
    return dbUser.update({pubg_username: PUBGUsername, pubg_id});
  }else{
    logger.log(`${user.username} not found, creating and setting PUBG data: ${PUBGUsername} (${pubg_id})`);
    ...
    return await Promise.all(
        users.map(async messageUser => {
          ...
          if (user) {
              ...
              logger.log(`${messageUser.username} found in DB, incrementing wins...`);
              return updatedUser.reload();
          } else {
              logger.log(`${messageUser.username} was not found in DB, creating user...`);
