...
const saveWinner = async(updated, winner) => {
  ...
  if(WINNERS.has(id)){
    ...
    const existingWin = WINNERS.get(id);
    existingWin.addPlayer(winner);
  }else{
    logger.info(`Not found in winners map, adding ${id}`);
    const WIN = new PUBGWin();
    WIN.addPlayer(winner);
    WINNERS.set(id, WIN);
  }
}
...
const handleWin = async(match, channel) => {
  ...
  const WIN = WINNERS.get(id);
  const {players} = WIN;
  logger.debug(`Found ${players.length} players in party: ${JSON.stringify(players)}`)
...
const cullStaleWinners = () => {
  const tenMinAgo = new Date(Date.now() - 10000 * 60).getTime();
  logger.debug(`WINNERS size: ${WINNERS.size} (pre-cull)`);
  for( let [key, value] of WINNERS ){
    if(value.timestamp < tenMinAgo){
      logger.debug(`WIN with ID ${key} stale, removing`);
      WINNERS.delete(key);
    }
  }
  logger.debug(`WINNERS size: ${WINNERS.size}`);
}
