...
const generateMatchHash = (presence) => {
  ...
  let partySize = presence.party.id;
  //solo doesn't have size prop
  if (presence.party.size) {
    partySize = presence.party.size.join();
  }
 ...
 * details example: Winner Winner Chicken Dinner!, Normal, Vikendi, 1/100
 * @param {*} details
 * @returns true if valid otherwise false
 */
const isValidMap = (details) => {
  const winningMode = details.split(", ");
  if (winningMode.length > 1 && VALID_MAPS.includes(winningMode[2])) {
    return true;
  }
  logger.info(`Ignoring mode [${winningMode[2]}] split from ${details}`);
  return false;
};
...
const getRandomResponse = (spectator) => {
  if (spectator) {
    return RESPONSES.SPECTATOR[Math.floor(Math.random() * RESPONSES.SPECTATOR.length)];
  }
  return RESPONSES.WINNER[Math.floor(Math.random() * RESPONSES.WINNER.length)];
};
