...
const handleWin = async(match, channel) => {
  ...
  const {alive, spectators} = separateAliveAndSpectators(winners);

  sendMessage(alive, channel);

  if(spectators.length > 0){
    sendMessage(spectators, channel, true);
  }
...
const separateAliveAndSpectators = (winners) => {
