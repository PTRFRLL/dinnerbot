...
const {ALLOWED_EXT, SIMPLE_DATE_FORMAT} = require('./constants');
...
/**
 * Generates timestamp in human readable format
 * @returns 
 */
const getTimestamp = (inputDate = '') => {
	let date = new Date();
	if(inputDate){
		date = typeof inputDate === 'string' ? new Date(inputDate) : inputDate;
	}
